...
```bash
docker run -v baserow_data:/baserow/data -p 80:80 -p 443:443 ghcr.io/carneirofc/baserow/baserow:0.14.0
```
...
docker run \
  ...
  ghcr.io/carneirofc/baserow/baserow:0.14.0
```
...
The `ghcr.io/carneirofc/baserow/baserow:0.14.0` image by default runs all of Baserow's various services in
...
docker run -it --rm -v baserow_data:/baserow/data ghcr.io/carneirofc/baserow/baserow:0.14.0 \
...
docker run -it --rm -v baserow_data:/baserow/data -v $PWD:/baserow/host \
   ghcr.io/carneirofc/baserow/baserow:0.14.0 backend-cmd-with-db backup -f /baserow/host/backup.tar.gz
```
...
docker run -it --rm \
  ...
  ghcr.io/carneirofc/baserow/baserow:0.14.0 backend-cmd-with-db restore -f /baserow/old_data/backup.tar.gz

# Or to restore from a file on your host instead run something like:
docker run -it --rm \
  -v baserow_data:/baserow/data -v \
  $(pwd):/baserow/host \
  ghcr.io/carneirofc/baserow/baserow:0.14.0 backend-cmd-with-db restore -f /baserow/host/backup.tar.gz
```
...
```dockerfile
FROM ghcr.io/carneirofc/baserow/baserow:0.14.0
