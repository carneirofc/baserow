...
The `baserow/baserow:2.3.2` image by default runs all of Baserow's various services in
...
docker run -it --rm -v baserow_data:/baserow/data baserow/baserow:2.3.2 \
...
docker run -it --rm -v baserow_data:/baserow/data -v $PWD:/baserow/host \
   baserow/baserow:2.3.2 backend-cmd-with-db backup -f /baserow/host/backup.tar.gz
```
...
docker run -it --rm \
  ...
  baserow/baserow:2.3.2 backend-cmd-with-db restore -f /baserow/old_data/backup.tar.gz

# Or to restore from a file on your host instead run something like:
docker run -it --rm \
  -v baserow_data:/baserow/data -v \
  $(pwd):/baserow/host \
  baserow/baserow:2.3.2 backend-cmd-with-db restore -f /baserow/host/backup.tar.gz
```
