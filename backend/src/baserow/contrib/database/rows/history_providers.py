...
class UpsertRowsFromFileHistoryProvider(BaseActionTypeRowHistoryProvider):
    """
    Row history for a file upsert: an update-style diff for every row the file
    matched and a create-style diff for every row it inserted.

    Only the rows the action actually recorded show up here. A large import is capped
    by `BASEROW_MAX_ROW_HISTORY_ENTRIES_PER_IMPORT`, and the import's
    `TableImportRecord` says so.
    """

    type = UpsertRowsFromFileActionType.type

    def get_changed_rows(
        self,
        command_type: ActionCommandType,
        params: UpsertRowsFromFileActionType.Params,
    ) -> Iterable[RowChangeData]:
        updated_values_by_id = {
            values["id"]: values
            for values in params.updated_rows_values
            if "id" in values
        }
        for row_id in params.updated_row_ids:
            metadata = params.updated_fields_metadata_by_row_id.get(row_id, {})
            yield RowChangeData(
                row_id=row_id,
                before=params.original_rows_values_by_id.get(row_id, {}),
                after=updated_values_by_id.get(row_id, {"id": row_id}),
                metadata=metadata,
            )

        created_values_by_id = {
            values["id"]: values
            for values in params.created_rows_values
            if "id" in values
        }
        for row_id in params.created_row_ids:
            metadata = params.created_fields_metadata_by_row_id.get(row_id, {})
            yield RowChangeData(
                row_id=row_id,
                before={"id": row_id, **dict.fromkeys(metadata.keys())},
                after=created_values_by_id.get(row_id, {"id": row_id}),
                metadata=metadata,
            )
    ...
    def get_row_history_entries(
        ...
        command_type: ActionCommandType,
        params: ActionData,
        action_params: UpsertRowsFromFileActionType.Params,
        row_change: RowChangeData,
        related_rows_diff: RelatedRowsDiff,
    ) -> list[RowHistory]:
        return _build_value_diff_entries(
            user,
            params,
            action_params.table_id,
            row_change,
            related_rows_diff,
            are_equal=(
                are_equal_on_create
                if row_change.row_id in action_params.created_row_id_set
                else None
            ),
        )


class ReplaceRowsFromFileHistoryProvider(BaseActionTypeRowHistoryProvider):
    """
    Row history for a file replace: a delete-style entry for every row that was
    removed and a create-style entry for every row the file inserted.
    """

    type = ReplaceRowsFromFileActionType.type

    def get_changed_rows(
        self,
        command_type: ActionCommandType,
        params: ReplaceRowsFromFileActionType.Params,
    ) -> Iterable[RowChangeData]:
        deleted_values_by_id = {
            values["id"]: values
            for values in params.deleted_rows_values
            if "id" in values
        }
        for row_id in params.deleted_row_ids:
            metadata = params.deleted_fields_metadata_by_row_id.get(row_id, {})
            yield RowChangeData(
                row_id=row_id,
                before=deleted_values_by_id.get(row_id, {"id": row_id}),
                after={"id": row_id, **dict.fromkeys(metadata.keys())},
                metadata=metadata,
            )
    ...
    def get_row_history_entries(
        ...
        command_type: ActionCommandType,
        params: ActionData,
        action_params: ReplaceRowsFromFileActionType.Params,
        row_change: RowChangeData,
        related_rows_diff: RelatedRowsDiff,
    ) -> list[RowHistory]:
        return _build_value_diff_entries(
...
def _build_value_diff_entries(
    ...
    """
    Turns one row's before/after values into its history entry, and records the
    matching diff for the tables linked to it.
    """

    row_id, before, after, metadata = row_change
    kwargs = {"are_equal": are_equal} if are_equal is not None else {}
    row_diff = extract_row_diff(table_id, row_id, metadata, before, after, **kwargs)
    if row_diff is None:
        return []
    ...
    update_related_tables_entries(related_rows_diff, changed_fields_metadata, row_diff)
    return [entry]
