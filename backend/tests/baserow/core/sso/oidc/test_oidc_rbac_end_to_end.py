"""
End-to-end cover for the OIDC group -> declared role -> permission manager chain.

Proves that an operator declaring a role in ``BASEROW_ROLES`` and mapping an IdP group
...
def test_oidc_group_grants_only_the_declared_roles_operations(data_fixture):
    ...
    # 2. ... and maps the IdP group "analysts" to it.
    ...
    # 3. The user logs in carrying that group.
...
@pytest.mark.django_db
def test_oidc_group_can_grant_an_operation_a_plain_member_lacks(data_fixture):
    ...
    plain_member_config = dataclasses.replace(
        ...
        workspace_mappings=[
            WorkspaceRoleMapping(
                group="analysts", workspace_id=workspace.id, role="MEMBER"
            )
        ],
    )
